...
### 17) Diğer işletim sistemlerine destek (macOS/Windows)
- **Bağlam (2026-09-04, kullanıcı):** "diğer işletim sistemlerine destek". Şu anki tüm mimari Linux+X11+GNOME'a kilitli: pencere spawn/yerleştirme (`gnome-terminal`, `xdotool`, `wmctrl`, `DISPLAY`), servis kalıcılığı (`systemd --user`), reboot tespiti (`/proc/uptime`), ekran-kilit kontrolü (`loginctl ... LockedHint`) — hiçbiri soyutlanmamış. TODO.md'deki "Wayland desteği için layout fallback" bile aynı X11-ailesinde kalıyor; macOS/Windows tamamen farklı bir sınıf.
- **Neyin GÖRECELİ KOLAY olduğu:** `tmux`'un kendisi cross-platform (macOS'ta native, Windows'ta WSL üzerinden) — `tmux_backend.py`'nin komut-satırı katmanı (new-session/capture-pane/send-keys) muhtemelen DEĞİŞMEDEN çalışır. `psutil` (proc-discovery/CPU) zaten cross-platform. Web panel (`py/cops web`, React arayüz, HTTP+WS) tamamen platform-bağımsız — tarayıcıdan erişim zaten OS'tan kopuk, `py/webui`'ye hiç dokunmaya gerek yok.
- **Neyin GERÇEKTEN OS-özel olduğu (soyutlanması gereken):** (a) terminal-window spawn (`spawn.py`: `gnome-terminal --window -- bash -c "..."` → macOS'ta `osascript`/Terminal.app veya iTerm2, Windows'ta `wt.exe`/PowerShell); (b) pencere yerleştirme/`layout` (`xdotool`/`wmctrl` → macOS'ta Accessibility API/`yabai` gibi 3. parti, Windows'ta Win32 API/PowerShell); (c) servis kalıcılığı (`systemd --user` → macOS `launchd`, Windows Task Scheduler/servis); (d) reboot/ekran-kilit tespiti (`/proc/uptime`, `loginctl` → platform-özel muadiller).
- **Olası strateji (henüz derinlemesine düşünülmedi):** GUI-pencere gerektiren KISIM (spawn'ın window-açma + layout) tamamen OPSİYONEL hale getirilip (spawn zaten `tmux_available()` yoksa/gnome-terminal başarısız olunca headless-tmux'a düşüyor — bu fallback path'i "birincil" yapmak) diğer OS'larda "hiç GUI penceresi yok, sadece tmux + web panel Terminal-view'i" modunda çalıştırmak — en az mühendislik, ama kullanıcının bugün alıştığı "gerçek pencereyi göreyim" kullanımını kaybettirir. Tam pencere-parite'si (spawn+layout ikisi de) her OS için ayrı bir "window backend" gerektirir — provider mimarisiyle AYNI desen (`WindowBackend` arayüzü + `gnome_terminal.py`/`macos_terminal.py`/`windows_terminal.py`), TBD#12'nin bash-silme kararına da dolaylı bağlı (worktree/mimari netleşmeden büyük bir yeni soyutlama katmanı eklemek erken olabilir).
- **Karar:** ? (TBD — hangi OS(lar) hedef, tam pencere-parite'si mi yoksa headless-only mod mu yeterli, ne zaman öncelik)

### 16) Aynı web UI'ı birden fazla makinede nasıl kullanırız?
- **Bağlam (2026-09-03, kullanıcı):** Şu an her makine kendi `py/cops web [--tunnel]`'ini bağımsız çalıştırıyor (bkz. TBD#6 kapanışı) — bu TEK bir makineye uzaktan erişim sağlıyor, birden çok makinedeki fleet'leri TEK ekranda birleştirmiyor.
- **Seçenekler (kabaca, henüz derinlemesine düşünülmedi):**
  (a) Her makine kendi paneliyle kalır, N tunnel URL'i N sekmede açılır — kod değişikliği yok, "aynı UI" tek pencerede değil ama iş görür.
  (b) Hafif bir aggregator: tek bir merkezi panel diğer makinelerin `/api/status`'unu (tunnel URL'leri üzerinden) çekip birleştirir — read-only kolay, aksiyon-tetikleme (start/stop/rc) daha karmaşık (hangi makineye proxy'lenecek, auth per-makine).
  (c) Push-model: her makine merkezi bir kayda kendi durumunu bildirir — gerçek dağıtık sistem, en ağır/en fazla mühendislik.
- **Karar:** ? (TBD)

### 15) Çoklu-CLI işbirliği: worker/checker/decider + paylaşımlı queue (MCP olarak)
- **Bağlam (2026-09-03, kullanıcı):** "İki veya daha fazla CLI'yı birbirine yardım edecek şekilde, birinin yaptığını diğeri kontrol edecek şekilde nasıl başlatırız?" Fikir: bitiren agent sonucu TİPİNİ belirterek bir queue/stack'e atar; başka bir agent onu kontrol edip kendi sonucunu yine queue'ya atar; bir üçüncüsü son kararı verir — Claude'un kendi exec-izni (permission-gate) yapısına benzer bir onay zinciri. Queue her CLI'ya bir **MCP server** olarak sunulur (push/pop/peek-by-type tool'ları); gruplu başlatmada bu MCP config'i + rollerin injection'ı otomatik olmalı (tmux ile mesaj gönderme zaten mümkün — bkz. remote-control/rc).
- **Literatür/prior art (araştırıldı, 2026-09-03):**
  - **Blackboard mimarisi** (Hearsay-II, 1980'ler) — fikrin akademik kökü: paylaşımlı bir "blackboard"a bağımsız "knowledge source"lar okur/yazar, bir "controller" sırayı belirler. Kullanıcının tarif ettiği queue+tip+kontrolcü tam olarak bu desen.
  - **MetaGPT** (ICLR 2024) — rol-bazlı ajanlar (PM/mimar/mühendis/QA) paylaşımlı bir mesaj havuzuna TİPLİ (`cause_by`/action tipi) mesaj yayınlıyor, ilgili rol tüketiyor — "tipini belirtip queue'ya atma" fikriyle neredeyse birebir.
  - **Anthropic'in kendi "Building Effective Agents" yazısı (2024)** — "evaluator-optimizer" pattern'i tam olarak "biri üretir, biri değerlendirir, döngü/karar" — en doğrudan eşleşme, üstelik aynı şirketin referans mimarisi.
  - **AutoGen/AG2 (Microsoft)**'in `GroupChatManager`'ı, **CrewAI**'nin hierarchical/manager process'i — üretici+eleştirmen+yönetici rollerini hazır orkestratör olarak sunan olgun/popüler OSS framework'ler (ama genelde TEK Python process İÇİNDE fonksiyon-çağrısı ajanlar için — claudeops'un yönettiği gibi bağımsız/kalıcı CLI process'leri için tasarlanmamışlar).
  - Akademik ek: "AI Safety via Debate" (Irving 2018, iki ajan + bir hakem), "Self-Refine"/"Reflexion" (ajan kendi/dış eleştiriyle iterasyon döngüsü).
  - **Bu session'ın kendi `Workflow` tool'u** (`pipeline()` + review→verify fazları) zaten AYNI pattern'i sağlıyor — ama Claude Code'un TEK session içi subagent'ları için, bizim fleet'teki gibi bağımsız/kalıcı ayrı CLI process'leri arası değil.
  - **Boşluk:** "N adet bağımsız/kalıcı CLI process'i (claude/agy/codex) + MCP-backed paylaşımlı queue + otomatik rol-injection'lı grup-başlatma" tam olarak bu şekilde paketlenmiş, tanınmış/hazır bir ürün YOK — bu bir DIY/niş inşa, hazır bir framework'ün doğrudan kopyası değil.
- **Öneri (uygulanmadı, sadece kayıt):** MCP server olarak basit bir sqlite/dosya-backed queue (push/pop/peek/by-type) + spawn'a `--role=worker|checker|decider` ve `--mcp-config` injection'ı eklemek en az sürpriz veren yol; MetaGPT'nin tipli-mesaj + Anthropic'in evaluator-optimizer'ı en yakın tasarım referansı.
- **Karar:** ? (TBD, henüz tasarıma geçilmedi)
...
- **Bulgular:**
  ...
  2. CLAUDE.md'nin kendi Faz 3 tarifi hâlâ `./claudeops layout ...` (BASH) diyor — `py/claudeops/commands/layout.py` docstring'inde kendini "bash'in karşılığı" tanımlasa da, gerçekten Faz 3'te (yani `--pin=`/`--group=`'lı gerçek bir handover akışında) CANLI denenip CLAUDE.md'nin `py/cops layout`'a çevrildiği DOĞRULANMADI; sadece dokümantasyon gecikmesi mi yoksa gerçek bir eksiklik mi bilinmiyor. **Hâlâ açık.** ~~`py/cops layout`'un kendisi hâlâ bilinen bir çoklu-monitor bug'ı taşıyor~~ — **DÜZELDİ (2026-09-03, b2dcfd7):** gerçek 10 pencerelik fleet üzerinde `py/cops layout` (pin/group'suz, düz çağrı) canlı doğrulandı, çoklu-monitor engeli kalktı (DONE.md). Kalan tek soru: aynı doğrulamanın `--pin=`/`--group=`'lı GERÇEK bir Faz 3 senaryosuyla da yapılması.
...
- **Öneri (silmeden önce sırayla):** (a) ~~autostart~~ **KAPANDI**; (b) Faz 3'ü gerçekten `py/cops layout`'la (pin/group'lu) denemek HÂLÂ AÇIK — çoklu-monitor engeli 2026-09-03'te kalktı, artık sadece "gerçek bir Faz 3 akışında dene" kaldı; (c) ~~komut denetimi~~ **KAPANDI** (yukarı bkz. — `compact` yapıldı, `desktops`/`send`-palette ayrı düşük-öncelikli TODO'lara döndü, `batch`/`self`/`new` gerek yok kararı verildi). Kalan TEK engel (b) — o kapanınca bash güvenle silinebilir.
- **Karar:** ? (a)+(c) KAPANDI, sadece (b) açık (artık sadece canlı-deneme eksik, teknik engel yok) — bash'in silinmesi buna bağlı.
