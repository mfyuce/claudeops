...
- **GÜNCELLEME (2026-08-31, aynı gün, ilerleyen saat):** İlk deneme merge (git merge auto-mode classifier tarafından reddedildi, denenmedi zaten) DEĞİL — kullanıcı kararı: **"merge etme şimdilik", ikisini PARALEL, ayrı tünellerle çalıştır, kullanıcı ikisini de canlı kullanıp karşılaştırsın**. Uygulandı: main (:8765, mevcut `claudeops-web.service`+`claudeops-tunnel.service`) DOKUNULMADI/aynı kaldı; react-ui için YENİ, ayrı bir kalıcı deploy kuruldu (:8766, `claudeops-web-react.service`+`claudeops-tunnel-react.service`, ikisi de systemd `--user`, reboot'ta otomatik — birebir main'in desenini taklit ediyor). İki tünel URL'i ayrı dosyalarda (`tunnel_url.txt` vs `tunnel_url-react.txt`), ntfy bildirimleri AYNI topic'i paylaşıyor ama mesaj metninde `[main]`/`[react]` etiketiyle ayrışıyor (`CLAUDEOPS_TUNNEL_LABEL`, `run-tunnel.sh`'e eklendi). İkisi de artık `server_started_at` alanı + client-side karşılaştırmayla redeploy sonrası açık sekmeleri OTOMATİK yeniliyor (kullanıcı isteği: "yenilenince de auto refresh"). **Bu süreçte bulunan/düzeltilen yan bug:** tunnel unit'i `Requires=` kullanıyordu — web.service'in HER restart'ı tunnel'ı da restart edip YENİ bir quick-tunnel URL'i üretiyordu (kullanıcının o an aktif kullandığı `various-diploma-insert-lynn` URL'i tam bu yüzden öldü); `Wants=`'a çevrildi, hem canlı deploy'da hem `service.py`'nin şablonunda (TÜM gelecekteki tek-instance kurulumları da kapsar).
- **Yeni systemd unit'ler REPO'DA TAKİP EDİLMİYOR** (`~/.config/systemd/user/claudeops-{web,tunnel}-react.service`, main'in `service.py install`'ının ürettiği unit'lerin AYNI deseninin elle/ad-hoc kopyası) — `service.py`'ye kalıcı, parametrik çoklu-instance desteği (ör. `--label`/`--port` flag'i) EKLENMEDİ (bilinçli: mevcut `run_install()` tek-instance'a hard-coded, riskli bir genelleştirme yerine bu ihtiyaç için minimal/izole bir çözüm tercih edildi). İleride 3. bir instance gerekirse ya bu iki unit'ten kopyalanır ya da `service.py` gerçekten genelleştirilir.
- **Soru (hâlâ açık):** Bir süre paralel kullanıp karşılaştırdıktan sonra merge edilsin mi, yoksa react-ui mi kalıcı "asıl" olsun (main deprecate) — ya da her ikisi de kalıcı mı kalsın? Kullanıcı henüz karar vermedi, sadece "merge etme şimdilik" dedi.
- **Karar:** ? (paralel çalışıyor, nihai yön açık)
