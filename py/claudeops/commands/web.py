...
def _start(name: str, model: str = "", permission_mode: str = "", effort: str = "", fresh: bool = False,
    ...
    # info["model"] eski cli'nin modeli — kullanıcı cli'yi DEĞİŞTİRİP model combosuna
    # dokunmazsa (placeholder value="" olarak kalır) burada YANLIŞ cli'nin modeliyle
    # spawn oluyordu (ör. codex'e geçip boş bırakınca "codex --model claude-sonnet-5"
    # gibi geçersiz bir çağrı — canlı kullanıcı raporu, 2026-09-01). cli değişmediyse
    # eski davranış (info["model"]) aynen korunur.
...
function render(d) {
  ...
  // AYNI "innerHTML= canlı DOM'u yok ediyor" kökeninden bir belirti daha: bir <select>
  // açıkken (native dropdown popup GÖRÜNÜRKEN) innerHTML= o düğümü DOM'dan söküp popup'ı
  // anında kapatıyordu (canlı kullanıcı raporu, 2026-09-01: "combo açıyorum ama refresh
  // oluyor ve combo kapanıyor"). xterm/input'un aksine burada "yakala+geri koy" İŞE
  // YARAMAZ — popup'ın kapanması node sökülürken tarayıcının kendi senkron davranışı,
  // input'un focus'unu geri verebildiğimiz gibi JS'ten geri açılamaz. Tek çözüm: seçim
  // sürerken bu tick'i tabContent için tamamen ATLAMAK (LAST_JSON hep güncellenir, bir
  // sonraki tick'te seçim bittiyse/vazgeçildiyse normal render devam eder).
  const tabContentEl = document.getElementById('tabContent');
  const activeSelectOpen = document.activeElement
    && document.activeElement.tagName === 'SELECT'
    && tabContentEl.contains(document.activeElement);
  if (!activeSelectOpen) tabContentEl.innerHTML = html;
  if (openXterm) {
