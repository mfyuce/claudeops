...
- **`rust/screenshare` (Uzak Masaüstü) — asla `pkill`/pattern-match ile öldürme** — HER ZAMAN `remote_desktop.stop()`/`.start()`, PID-exact. **v2'den itibaren input enjeksiyonu var** ("Kontrolü Al", varsayılan KAPALI) — GERÇEK fare/klavyeyi fiziksel kullanıcıyla paylaşır (X11 click/scroll imleç KONUMUNA göre yönlenir, focus'a değil), kilitli ekranı şifre yazıp açabilir (kasıtlı). Ctrl/Alt/⌘ kombinasyonları YOK. [[remote-desktop-screenshare-v1]] [[pkill-pattern-kills-live-daemon]]
