...
- [ ] **KRİTİK: rc/handover guard.lock'u KENDİ alsın + KAYIT bitene kadar tutsun** (TODO-h+j birleşik, 2026-06-09 *37→*38'de PATLADI) — `flock -u`'yu komut bitince yapmak YETMEZ: RC bridge kaydı dakikalarca sürebiliyor (claude.ai yükü); release sonrası kayıtsız pencerede guard yarışıp HER session'ı dupledi (Faz1 12, Faz2 22 = 44 proc). Fix: `cmd_rc`/`cmd_handover` başında `exec 9>guard.lock; flock 9`, spawn'lardan sonra **`list`'te beklenen N session kayıtlı olana kadar poll** (timeout ~5dk), SONRA `flock -u`. Şimdilik elle: background lock-holder (`nohup bash -c 'exec 9>...; flock 9 && sleep 900' &`). Memory [[handover-edge-cases]] edge-9.
