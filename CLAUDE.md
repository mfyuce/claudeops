...
**Yeni: `py/cops web [--port 8765] [--tunnel]`** — yerel kontrol paneli. Roster'ın TAMAMINI (aktif/kapalı/emekli) tablo halinde gösterir, mass-start YOK, her isim için tek tek "başlat" (model/permission-mode/effort/fresh seçenekleriyle) / "durdur" / "emekli et" (kill + models.tsv+roster.tsv yorumla) / "tekrar işe al+başlat" (uncomment + spawn). Token-gated (`~/.claude/claudeops/web.token`, ilk çalıştırmada random üretilir) — `--tunnel` ile `cloudflared` quick-tunnel açıp dışarıdan (telefon vb.) erişilebilir hale getirir (URL her başlatmada değişir, `cloudflared` `~/.local/bin`'e kuruldu). Foreground komut — Ctrl-C ile hem server hem tünel kapanır; arka planda bırakmak için `nohup ... & disown`.
