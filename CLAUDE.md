...
✅ **guard watchdog + oomd kurtarma** (DONE 2026-06-03, sertleştirme 2026-06-04 f4d949f): fleet topluca kapanırsa (`systemd-oomd` gnome-terminal-server.service cgroup'unu SIGKILL — journal'da, dmesg'te değil; tek-cgroup mimarisi=tek nokta arıza) `claudeops guard` idempotent geri açar (isim VEYA cwd eşleşmesi → skip; **HER ZAMAN en güncel jsonl resume** — kullanıcı "önceki/son session'dan devam"). Cron `*/2` + autostart `guard --boot --lock`; `_detect_display` ile DISPLAY oto-tespit. **2026-06-04 OOM-recovery 5 kök bug fix:** (1) `SPAWN_ENV` PATH — gnome-terminal child SERVER env'i miras alır, cron yeni server'ı minimal-PATH'le açınca `claude: command not found`; (2) `all_sessions_tsv` pid-liveness (`os.kill`+`procStart`) — SIGKILL stale json'u "çalışıyor" sanıp `reopened=0`; (3) suffix-gate kaldırıldı (idle/crash fresh açıp context kaybediyordu); (4) `_latest_sid_for_cwd` `agent-*` exclude+UUID-only (`resume=agent-a2` çöp); (5) `flock -n` concurrent-guard duplicate. **Handover ETKİLENMEZ** (fresh-aç ayrı komut `rc --new`; guard çalışan session'a dokunmaz). Detay [[oomd-cgroup-kill]]. **Kullanıcı kararı: oomd'ye dokunma, guard kurtarsın.**
